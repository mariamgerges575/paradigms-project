...
class Connect4Gamee() {
  ...
  def chessController (gameState:state,input:String):Option[state]={
    ...
    def validateMove(gameState: state,arrInput:Array[Int]): Boolean = {
    ...
    def applyMove(gameState:state,arrInput:Array[Int]): state = {
      val fromRow =arrInput(0)
  ...
  def connect4Controller(gameState:state,input:String):Option[state]={
      def applyMove(gameState:state,col:Int):state ={
    ...
    def checkersController(gameState:state,input:String):Option[state]={

      def isValidMove(gameState:state,arrInput:Array[Int]):Boolean = {
  ...
  def sudokuController(gameState: state, input: String): Option[ state ]= {

    def isValidGame(gameState: state,  r: Int, c: Int, value: Int): Boolean = {
    ...
    def applyChange(gameState: state, r: Int, c: Int, value: Int): state = {
      val newState: state = (gameState._1.clone(), switchPlayers(gameState._2))
  ...
  def tictactoeController(gameState:state,input:String):Option[state]={
      def validTTmove(gameState: state, row:Int,col:Int): Boolean = {
        if ( gameState._1(row)(col) == '❌' || gameState._1(row)(col) == '⭕' ) false else true
      }
      def applyMoveTT(gameState: state, row:Int,col:Int): state = {
    ...
    def eightQueensController(gameState:state,input:String):Option[state]={

      def validMove(gameState: state,row:Int,col:Int):Boolean={
        //validate row
      ...
      def applyMove(gameState: state,row:Int,col:Int):state=
      {
